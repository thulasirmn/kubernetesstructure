...
public sealed class KubernetesOrchestrator : IKubernetesOrchestrator
{
        ...
        CancellationToken ct = default)
    {
        ...
        var commandArr = application.Type == ApplicationType.RStudio
            ? BuildRStudioCommand(ingressPath)
            : ParseCommand(application.CommandJson, basePath);
        ...
        var container = new V1Container
        {
            ...
            VolumeMounts = new List<V1VolumeMount>
            {
                new()
                {
                    ...
                    // No SubPath — every user mounts the share root and sees the same files.
                }
            },
        ...
        var ingress = new V1Ingress
        {
            Metadata = new V1ObjectMeta
            {
                ...
                NamespaceProperty = workspace.K8sNamespace,
                Labels = labels,
                Annotations = BuildIngressAnnotations(application.Type)
            },
            Spec = new V1IngressSpec
            {
                ...
                {
                    BuildIngressRule(_azureOptions.IngressDomain, ingressPath, session.ServiceName)
                }
            }
        };
        await _client.NetworkingV1.CreateNamespacedIngressAsync(ingress, workspace.K8sNamespace, cancellationToken: ct);

        var url = BuildAccessUrl(_azureOptions.IngressDomain, ingressPath);
        return new SessionDeploymentResult(session.DeploymentName, session.ServiceName, url);
    }

    private static V1IngressRule BuildIngressRule(string ingressDomain, string ingressPath, string serviceName)
    {
        ...
        // K8s rejects IPs as Host values — only set Host for valid DNS names.
        if (!string.IsNullOrWhiteSpace(ingressDomain) && !System.Net.IPAddress.TryParse(ingressDomain, out _))
        {
            rule.Host = ingressDomain;
        }
        return rule;
    }

    private static string BuildAccessUrl(string ingressDomain, string ingressPath)
    {
        if (string.IsNullOrWhiteSpace(ingressDomain))
            return ingressPath + "/";
        // For dev (IP-based ingress), use http://. For real DNS names, use https://.
        var scheme = System.Net.IPAddress.TryParse(ingressDomain, out _) ? "http" : "https";
        return $"{scheme}://{ingressDomain}{ingressPath}/";
    }
    ...
    private static Dictionary<string, string> BuildIngressAnnotations(ApplicationType appType)
    {
        var annotations = new Dictionary<string, string>
        {
            ...
            // Required for the regex path "(/|$)(.*)" to be interpreted as a regex.
            ["nginx.ingress.kubernetes.io/use-regex"] = "true"
        };

        // RStudio's HTML references /page.css etc. so the ingress must strip the /s/<slug>
        // prefix; we tell RStudio its public root via www-root-path (see BuildRStudioCommand).
        // Jupyter, in contrast, is launched with --ServerApp.base_url=/s/<slug>/ and so
        // generates URLs that already include the prefix — stripping it would 404 every asset.
        // Custom apps default to no-strip (matches Jupyter's "app handles its own base path").
        if (appType == ApplicationType.RStudio)
        {
    ...
    /// <summary>
    /// Wraps the rocker/rstudio entrypoint so we can declare www-root-path in rserver.conf
    /// before s6-init runs. We append to BOTH rserver.conf and disable_auth_rserver.conf
    /// because the image's userconf script may overwrite rserver.conf with the latter when
    /// DISABLE_AUTH=true. Whichever config rserver ends up reading, www-root-path is set,
    /// so RStudio prefixes its asset URLs with /s/&lt;slug&gt;/.
    /// </summary>
    private static IList<string> BuildRStudioCommand(string ingressPath)
    {
        var line = $"www-root-path={ingressPath}";
        var script =
            $"printf '%s\\n' '{line}' >> /etc/rstudio/rserver.conf 2>/dev/null || true; " +
            $"if [ -f /etc/rstudio/disable_auth_rserver.conf ]; then printf '%s\\n' '{line}' >> /etc/rstudio/disable_auth_rserver.conf; fi; " +
            "exec /init";
        return new[] { "/bin/bash", "-c", script };
    }
